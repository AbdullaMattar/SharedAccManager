...
export default function Users() {
  ...
  return <div className="space-y-5"><div className="flex justify-between"><h1 className="text-2xl font-bold">{strings.phase3.users}</h1><Button onClick={() => edit()}><Plus className="me-2 h-4 w-4" />{strings.phase3.addUser}</Button></div>
    <div className="grid gap-3 md:grid-cols-2">{data.map((user) => <Card key={user.id}><CardContent className="flex items-center justify-between gap-3 p-4"><div><strong>{user.name}</strong><p className="text-sm text-muted-foreground">{user.email}</p><small>{user.role === "admin" ? strings.phase3.roleAdmin : strings.phase3.roleStaff} · {user.enabled ? strings.phase3.enabled : strings.phase3.disabled}</small></div><div><Button size="icon" variant="ghost" onClick={() => edit(user)}><Edit2 className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={() => setResetUser(user)}><KeyRound className="h-4 w-4" /></Button></div></CardContent></Card>)}</div>
