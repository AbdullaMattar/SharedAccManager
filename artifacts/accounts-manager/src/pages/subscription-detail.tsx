...
import { SubscriptionSummary, useCancelSubscription, useGetSubscription, useUpdateSubscription } from "@/lib/phase2-api";
import { strings } from "@/lib/strings";

type SubscriptionDetailData = SubscriptionSummary & { customerId: number; payments?: { id: number; amount: number; method: string; paidAt: string; notes?: string }[]; slotHistory?: SubscriptionSummary[] };
...
export default function SubscriptionDetail({ id }: { id: number }) {
  ...
  const cancelSubscription = () => cancel.mutate({ id }, { onSuccess: () => { invalidate(); toast({ title: strings.subscriptions.cancelSuccess }); } });
  return <div className="space-y-5">
    ...
    <Card><CardHeader><div className="flex items-start justify-between"><div><CardTitle>{value.productName}</CardTitle><Link href={`/customers/${value.customerId}`} className="text-sm text-primary">{value.customerName}</Link></div><SubscriptionStatusBadge status={value.status} /></div></CardHeader><CardContent className="space-y-4"><div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4"><Fact label={strings.subscriptions.account} value={value.accountLabel} /><Fact label={strings.subscriptions.slot} value={String(value.slotIndex)} /><Fact label={strings.subscriptions.startDate} value={value.startDate} /><Fact label={strings.subscriptions.expiryDate} value={value.expiryDate} /><Fact label={strings.subscriptions.price} value={`${value.price} ${strings.common.currency}`} /></div>
